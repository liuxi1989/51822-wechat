...
//wechat cha uuid define
#define BLE_UUID_WECHAT_CURRENT_PEDOMETER_MEASUREMENT 0xFEA1  //current_pedometer_measurement
#define BLE_UUID_WECHAT_TARGET 0xFEA2  //target
#define BLE_UUID_WECHAT_MACADD							 			0xFEC9
...
typedef enum
{
    BLE_STEP_EVT_NOTIFICATION_ENABLED,                             /**< Battery value notification enabled event. */
    BLE_STEP_EVT_NOTIFICATION_DISABLED                             /**< Battery value notification disabled event. */
} ble_step_evt_type_t;

/**@brief Battery Service event. */
typedef struct
{
    ble_step_evt_type_t evt_type;                                  /**< Type of event. */
} ble_step_evt_t;

// Forward declaration of the ble_bas_t type. 
typedef struct ble_step_s ble_step_t;

/**@brief Battery Service event handler type. */
typedef void (*ble_step_evt_handler_t) (ble_step_t * p_bas, ble_step_evt_t * p_evt);

/**@brief Battery Service init structure. This contains all options and data needed for
 *        initialization of the service.*/
typedef struct
{
    ble_step_evt_handler_t         evt_handler;                    /**< Event handler to be called for handling events in the Battery Service. */
    bool                          support_notification;           /**< TRUE if notification of Battery Level measurement is supported. */
    ble_srv_report_ref_t *        p_report_ref;                   /**< If not NULL, a Report Reference descriptor with the specified value will be added to the Battery Level characteristic */
    uint8_t                       initial_step_count;             /**< Initial battery level */
    ble_srv_cccd_security_mode_t  step_count_char_attr_md;     /**< Initial security level for battery characteristics attribute */
    ble_gap_conn_sec_mode_t       step_count_report_read_perm; /**< Initial security level for battery report read attribute */
} ble_step_init_t;
...
struct ble_step_s
{
    ...
    ble_gatts_char_handles_t      step_count_handles;          /**< Handles related to the Battery Level characteristic. */
    uint16_t                      report_ref_handle;              /**< Handle of the Report Reference descriptor. */
    uint8_t                       step_count_last;             /**< Last Battery Level measurement passed to the Battery Service. */
 ...
 */
uint32_t ble_step_init(ble_step_t * p_bas, const ble_step_init_t * p_bas_init);
 ...
 */
void ble_step_on_ble_evt(ble_step_t * p_bas, ble_evt_t * p_ble_evt);
